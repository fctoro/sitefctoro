...
export function HomePageContent({
  ...
  const players = initialPlayers.length > 0 ? initialPlayers : playerCards
  const allNews = initialNews
  const heroSlides = initialSlides.length > 0 ? initialSlides : fallbackSlides
  ...
  useEffect(() => {
    if (!introReady || showIntro || heroSlides.length <= 1) return
  ...
  }, [heroSlides.length, introReady, showIntro])

  useEffect(() => {
    if (activeHero >= heroSlides.length) {
      setActiveHero(0)
    }
  ...
  const activeHeroSlide = heroSlides[activeHero] || fallbackSlides[0]
  const activeHeroTitle = activeHeroSlide.title

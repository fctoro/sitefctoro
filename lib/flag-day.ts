  ...
  try {
    ...
    const { rows: scorerRows } = await pool.query<ScorerRow & { team_logo_url: string | null; competition_id: string }>(
      `
        ...
        inner join flagday_categories cat on cat.id = s.category_id
    ...
    const aggregatedScorersMap: Record<string, { player_name: string; team_name: string; goals: number; team_logo_url: string | null; competition_id: string }> = {}

    scorerRows.forEach((scorer) => {
      const playerName = scorer.player_name?.trim()
      const teamName = scorer.team_name?.trim()
      const compId = scorer.competition_id

      if (!playerName || !teamName || !compId) {
        return
      }
